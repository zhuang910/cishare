...
class Society_Model extends CI_Model {
	...
	function get_achievement($userid,$nowterm,$scoretype,$identifying){
		$id=$this->get_student_id($userid);
		if(!empty($id)){
			$this->db->select('score.score,course.name,course.englishname,major.name as mname,major.englishname as menname,squad.name as sname,squad.englishname as senname');
			$this->db->where('studentid',$id);
