 ...
 *        
 */
class Acc_camp_Model extends CI_Model {
	...
	const T_CAMPUS_INFO='school_accommodation_campus_info';//校区信息表
	...
	function count($condition, $programaids = null) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			...
			return $this->db->from ( self::T_CAMPUS )->count_all_results ();
		}
		return 0;
	}
	...
	function get($field, $condition, $programaids = null) {
		if (is_array ( $field ) && ! empty ( $field )) {
			...
			return $this->db->get ( self::T_CAMPUS )->result ();
		}
		return array ();
	}
	...
	function get_one($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->limit ( 1 )->get ( self::T_CAMPUS )->row ();
			if ($base) {
				return $base;
			}
			return array ();
	...
	function get_info_one($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->limit ( 1 )->get ( self::T_CAMPUS_INFO )->row ();
			if ($base) {
				return $base;
			}
			return array ();
	...
	function save($id = null, $data = array()) {
		if (! empty ( $data )) {
			if ($id == null) {
				$this->db->insert ( self::T_CAMPUS, $data );
				return $this->db->insert_id ();
			} else {
				return $this->db->update ( self::T_CAMPUS, $data, 'id = ' . $id );
			}
		}
	}
	/**
	...
	function save_info($id = null, $data = array()) {
		if (! empty ( $data )) {
			$where = 'columnid='.$data['columnid'].' AND site_language="'.$data['site_language'].'"';
			$num=$this->get_info_one($where);
			if(!empty($num)){
				$this->db->delete ( self::T_CAMPUS_INFO, $where );

			}
			if ($id == null) {
				$data['createtime']=time();
				$data['adminid']=$_SESSION['master_user_info']->id;
				$this->db->insert ( self::T_CAMPUS_INFO, $data );
				return $this->db->insert_id ();
			}
		}
	}
	...
	function delete($where = null) {
		if ($where != null) {
			$this->db->delete ( self::T_CAMPUS, $where );
			return true;
		}
		return false;
	}
