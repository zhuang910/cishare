 ...
 *        
 */
class Barter_card_Model extends CI_Model {
	...
	function get($field, $condition) {
		if (is_array ( $field ) && ! empty ( $field )) {
			...
			return $this->db->get ( self::T_STU_BAR_CARD )->result ();
		}
		return array ();
	}
	...
	function count($condition) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			...
			return $this->db->from ( self::T_STU_BAR_CARD )->count_all_results ();
		}
		return 0;
	}
	...
	function delete($where) {
		if ($where != null) {
			if ($this->db->delete ( self::T_STU_BAR_CARD, $where)) {
				return true;
			}
		}
		return false;
	...
	function get_one($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->limit ( 1 )->get ( self::T_STU_BAR_CARD )->row ();
			if ($base) {
				return $base;
			}
			return array ();
	...
	function save($id = null, $data = array()) {
		if (! empty ( $data )) {
			if ($id == null) {
				if ($this->db->insert ( self::T_STU_BAR_CARD, $data )) {
					
					return $this->db->insert_id ();
				}
			} else {
				$this->db->update ( self::T_STU_BAR_CARD, $data, 'id = ' . $id );
			}
		}
	}
	...
	function get_remark($id){
		if(!empty($id)){
			...
			$this->db->where('id',$id);
			$data=$this->db->get(self::T_STU_BAR_CARD)->row_array();
			return $data['remark'];
		}
	}
	/**
