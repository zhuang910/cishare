...
class Fillingoutforms_Model extends CI_Model {
	...
	function get_course_info($cid = 0) {
		if ($cid) {
			return $this->db->select ( 'id,name,opentime,endtime,tuition,applytuition,isapply,applytemplate,attatemplate,state' )->where ( 'id = ' . $cid )->limit ( 1 )->get ( self::T_COURSE )->row ();
		}
		return false;
	}
	...
	function get_apply_info($cid = 0, $uid = 0) {
		if ($cid && $uid) {
			...
			return $this->db->select ( 'id,number,paystate,isstart,isinformation,isatt,issubmit,state' )->where ( 'userid = ' . $uid . ' AND courseid = ' . $cid )->limit ( 1 )->get ( self::T_APPLY )->row ();
		}
		return false;
	}
}