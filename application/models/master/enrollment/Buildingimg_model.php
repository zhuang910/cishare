 ...
 *        
 */
class Buildingimg_Model extends CI_Model {
	...
	function count($condition, $programaids = null) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			...
			return $this->db->from ( self::T_PICTURES )->count_all_results ();
		}
		return 0;
	}
	...
	function get($field, $condition, $programaids = null) {
		if (is_array ( $field ) && ! empty ( $field )) {
			...
			return $this->db->get ( self::T_PICTURES )->result ();
		}
		return array ();
	}
	...
	function get_one($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->limit ( 1 )->get ( self::T_PICTURES )->row ();
			if ($base) {
				return $base;
			}
			return array ();
	...
	function save($id = null, $data = array()) {
		if (! empty ( $data )) {
			if ($id == null) {
				$this->db->insert ( self::T_PICTURES, $data );
				return $this->db->insert_id ();
			} else {
				return $this->db->update ( self::T_PICTURES, $data, 'id = ' . $id );
			}
		}
	}
	...
	function delete($where = null) {
		if ($where != null) {
			$this->db->delete ( self::T_PICTURES, $where );
			return true;
		}
		return false;
	}
