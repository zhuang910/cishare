...
class Pickup_Model extends CI_Model {
	...
	function count($condition, $programaids = null) {
		...
		function counts($condition, $programaids = null) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			if (! empty ( $condition ['where'] )) {
				$this->db->where ( $condition ['where'] );
			}
			...
			return $this->db->from ( 'acc_info' )->count_all_results ();
		}
		return 0;
	}
	
		function gets($field, $condition, $programaids = null) {
		if (is_array ( $field ) && ! empty ( $field )) {
			...
			return $this->db->get ( 'acc_info' )->result ();
		}
		return array ();
	}
	/**
