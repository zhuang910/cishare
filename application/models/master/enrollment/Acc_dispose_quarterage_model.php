 ...
 *        
 */
class Acc_dispose_quarterage_Model extends CI_Model {
	...
	function count($where=null) {
    	$this->db->select('accommodation_info.*,zust_student_info.chname,zust_student_info.passport,zust_student_info.nationality,zust_student_info.email');
		if (! empty ( $where )) {
			$this->db->where ( $where );
		}
		$this->db->where('accommodation_info.acc_state',6);
    	$this->db->join('zust_student_info','zust_student_info.id=accommodation_info.userid');
	...
	function get($where = null, $limit = 0, $offset = 0, $orderby = 'id desc') {
    	$this->db->select('accommodation_info.*,zust_student_info.enname,zust_student_info.passport,zust_student_info.nationality,zust_student_info.email');
		if (! empty ( $where )) {
