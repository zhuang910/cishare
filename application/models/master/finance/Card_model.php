...
class Card_Model extends CI_Model{
	...
	function count($where = null){
    	...
    	$this->db->join('zust_slot_card','zust_student_info.id=zust_slot_card.userid');
    ...
    function get($where = null,$limit = 25,$offset = 0,$orderby = null){
    	$this->db->select('zust_slot_card.*,zust_student_info.chname,zust_student_info.enname,zust_student_info.passport');
    	if($where != null){
			$this->db->where($where);
    	}
    	if($limit){
    	...
    	$this->db->join('zust_student_info','zust_student_info.id=zust_slot_card.userid');
