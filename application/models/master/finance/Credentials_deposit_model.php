...
class Credentials_deposit_Model extends CI_Model {
	...
	function count($condition) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
            ...
            $this->db->join ( self::T_STU_INFO, self::T_STU_INFO . '.id=' . self::T_A_O_I . '.userid' );
            $this->db->join ( self::T_APP_ORDER, self::T_APP_ORDER . '.id=' . self::T_A_O_I . '.orderid' );
            $this->db->where('credentials.ordertype = 5');
