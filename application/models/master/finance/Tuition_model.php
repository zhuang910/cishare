...
class Tuition_Model extends CI_Model{
	...
	//查询数据总数
	function count($where = null,$major_id,$term,$squad_id){
    	$this->db->select('zust_tuition_info.*,zust_student.*');
		if($where !== null){
			$this->db->where($where);
    	}
    	if($major_id!=0){
    	...
    	if($term!=0){
    		$this->db->where('zust_tuition_info.nowterm',$term);
    	}
    	if($squad_id!=0){
    		$this->db->where('zust_student.squadid',$squad_id);
    	}
    	$this->db->join('zust_student','zust_student.userid=zust_tuition_info.userid');
...
//获得数据
    function get($where = null,$limit = 25,$offset = 0,$orderby = null,$major_id,$term,$squad_id){
    	$this->db->select('zust_tuition_info.*,zust_tuition_info.id as bookid,zust_student.*');
    	if($where != null){
			$this->db->where($where);
    	}
    	if($limit){
