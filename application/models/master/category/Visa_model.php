...
class Visa_Model extends CI_Model {
     ...
     * @return array
     */
	function get($field, $condition,$label_id =1) {
		if (is_array ( $field ) && ! empty ( $field )) {
			
			$this->db->select ('student_visa.id, student_visa.visatime,student_visa.manage_state,student.id,student.name,student.majorid,student.squadid,student.mobile,student.enname,student.passport');
			...
			if (is_array ( $condition ) && ! empty ( $condition )) {
                ...
                if($label_id == 1){ // 两周预警
                   ...
                   // $end = mktime(0,0,0,date('m'),date('d')+14,date('Y'));
                   // $end = date('Y-m-d',$end);

                   // $this->db->where('student_visa.visatime > \''.$start.'\' AND student_visa.visatime <=\''.$end.'\'');
				  
				   $this->db->where('student_visa.visatime > 0 AND student_visa.visatime > \''.$s.'\' AND student_visa.visatime <=\''.$s1.'\'');
                }
                ...
                if($label_id == 2){ // 一周预警
                    ...
                    $this->db->where('student_visa.visatime > 0 AND student_visa.visatime > \''.$s.'\' AND student_visa.visatime <=\''.$s2.'\'');
                }

                if($label_id == 3){ // 已经过期
                    $time = date('Y-m-d',time());
                    $this->db->where('student_visa.visatime > 0 AND student_visa.visatime <=\''.$s.'\'');
                }
     ...
     * @return int
     */
	function count($condition,$label_id=1) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			if (! empty ( $condition ['where'] )) {
				$this->db->where ( $condition ['where'] );
			}
		...
		if($count>0){
			if(!empty($data)&&!empty($name)){
				if($name=='visatime'){
					...
					$this->db->update(self::T_STUDENT_VISA,array('visatime'=>$date),'studentid='.$data['pk']);
				}else if($name=='oldvisatime'){
					$date=strtotime($data['value']);
					$this->db->update(self::T_STUDENT_VISA,array('oldvisatime'=>$date),'studentid='.$data['pk']);
				}else if($name=='issuetime'){
					$date=strtotime($data['value']);
					$this->db->update(self::T_STUDENT_VISA,array('issuetime'=>$date),'studentid='.$data['pk']);
				}else{
					$arr[$name]=$data['value'];
