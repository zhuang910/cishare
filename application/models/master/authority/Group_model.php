...
class Group_Model extends CI_Model {
	const T_G = 'ci_admin_info';
	const T_ARTICLE = 'ci_system_group';
	...
	function get_group($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->get ( self::T_G )->row ();
			if ($base) {
				foreach ( $base as $k => $v ) {
					$data [$v ['groupid']] = $v ['title'];
				}
				return $data;
			}
			return array ();
	 ...
	 * 删除权限
	 */
	function delpower($where = null) {
		if ($where != null) {
			$this->db->delete ( self::T_M, $where );
			return true;
		}
		return false;
	}
	 ...
	 * 保存权限
	 */
	function savepower($data = null) {
		if ($data != null) {
			return $this->db->insert ( self::T_M, $data );
		}
	}
	...
	function get_power($where = null) {
		if ($where != null) {
			$base = array ();
			$base = $this->db->where ( $where )->get ( self::T_M )->result_array ();
			if ($base) {
				return $base;
			}
			return array ();
