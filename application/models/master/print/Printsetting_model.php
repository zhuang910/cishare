...
class Printsetting_Model extends CI_Model {
	 ...
	 * @param array $condition        	
	 */
	function count_fields($condition,$print_templateid=null) {
		if (is_array ( $condition ) && ! empty ( $condition )) {
			if (! empty ( $condition ['where'] )) {
				$this->db->where ( $condition ['where'] );
			}
			if($print_templateid!=null){
				$this->db->where('print_templateid',$print_templateid);
			}
	 ...
	 * @param array $condition        	
	 */
	function get_fields($field, $condition,$print_templateid=null) {
		if (is_array ( $field ) && ! empty ( $field )) {
			$this->db->select ( str_replace ( " , ", " ", implode ( "`, `", $field ) ) );
			if (is_array ( $condition ) && ! empty ( $condition )) {
				if (! empty ( $condition ['where'] )) {
