...
class Accommodation_Model extends CI_Model {
	...
	/**
	 * [get_student_info 获取已经预订学生的信息]
	 * @return [type] [description]
	 */
	function get_student_info($rid){
		if(!empty($rid)){
			//获取已经预订成功的学生
			$acc_info=$this->get_success_room($rid);
			$stu_arr=array();
			if(!empty($acc_info)){
				foreach ($acc_info as $k => $v) {
					//获取学生的信息
					$stu_arr[$v['userid']]=$this->get_user_info_one($v['userid']);
					$stu_arr[$v['userid']]['remark']=$v['remark'];
				}
			}
			return $stu_arr;
		}
	...
	/**
	 * [get_success_room 获取成功订房的信息]
	 * @return [type] [description]
	 */
	function get_success_room($rid){
		if(!empty($rid)){
			$this->db->where('roomid',$rid);
			$this->db->where('paystate <> 0 AND paystate <> 2');
			$this->db->where('acc_state <> 4 AND acc_state <> 7');
			return $this->db->get(self::T_ACC)->result_array();
		}
	}
	/**
	 * [get_user_info 获取学生信息]
	 * @return [type] [description]
	 */
	function get_user_info_one($id){
		if(!empty($id)){
			$this->db->where('id',$id);
			return $this->db->get(self::T_STUDENT_INFO)->row_array();
		}
