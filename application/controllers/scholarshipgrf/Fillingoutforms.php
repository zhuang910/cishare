...
class FillingOutForms extends Student_Basic {
	...
	function apply() {
		...
		if (! empty ( $applyid )) {
			$where_apply_info = "id = {$applyid} AND userid = {$_SESSION['student'] ['userinfo'] ['id']} AND type = 2";
			// 首先要更改 isstart = 1
			...
			if ($cid) {
				...
				if (empty ( $state )) {
					redirect ( 'http://' . $_SERVER ['HTTP_HOST'] . '/' . $this->puri . '/scholarship/apply?id=' . cucas_base64_encode ( $cid ) );
				}
				...
				$coursenames = $this->scholarship_model->get_one ( 'id = ' . $cid );
				// 生成表单html
				...
				$this->_view ( 'fill_form', array (
						...
						'courseid' => $apply_info ['scholarshipid'],
		...
		if (IS_AJAX) {
			...
			if ($cid) {
				...
				if (! empty ( $is )) {
					...
					$url = $ischeck == 'ischeck' ? '/' . $this->puri . '/scholarshipgrf/apply/upload_materials?applyid=' . $c_id : '';
					if ($ischeck) {
