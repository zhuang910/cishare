 ...
 *        
 */
class Stuscholarship extends Student_Basic {
	...
	function index() {
		...
		*/
		$result = $this->db->select ( '*' )->get_where ( 'scholarship_info', 'id > 0 AND state = 1 AND apply_state = 1 AND ischinascholorship = 0' )->result_array ();
		//var_dump($result);
	...
	function apply() {
		...
		if ($id) {
			...
			switch ($flag) {
				...
				case 2 :
					// 已经申请了
					echo '<script>window.location.href="/' . $this->puri . '/scholarshipgrf/stuscholarship/myscholarship"</script>';
					die ();
					break;
				case 1 :
					// 可以申请
	 ...
	 * 判断权限
	 */
	function _apply_authoritys($id = null) {
		if ($id != null) {
			// 查看信息
			$result = $this->db->select ( '*' )->get_where ( 'scholarship_info', 'id = ' . $id . ' AND state = 1 AND apply_state = 1 AND ischinascholorship = 0' )->row ();
			if (! empty ( $result )) {
