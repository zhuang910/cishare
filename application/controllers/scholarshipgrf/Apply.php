			...
			if (! empty ( $apply_info )) {
				...
				if ($issubmit == 0) {
					...
					if ($apply_info ['isstart'] == 0) {
						$jumpurl = '/' . $this->puri . '/scholarshipgrf/apply?id=' . cucas_base64_encode ( $id );
						return $data = array (
								'jumpurl' => $jumpurl,
								'flag' => 2 
						);
					...
					if ($apply_info ['isinformation'] == 0) {
						$jumpurl = '/' . $this->puri . '/scholarshipgrf/fillingoutforms/apply?applyid=' . cucas_base64_encode ( $apply_info ['id'] );
						return $data = array (
								'jumpurl' => $jumpurl,
								'flag' => 1 
						);
					...
					if ($apply_info ['isatt'] == 0) {
						$jumpurl = '/' . $this->puri . '/scholarshipgrf/apply/upload_materials?applyid=' . cucas_base64_encode ( $apply_info ['id'] );
						return $data = array (
								'jumpurl' => $jumpurl,
								'flag' => 1 
						);
					...
					$jumpurl = '/' . $this->puri . '/scholarshipgrf/apply/submit_application?applyid=' . cucas_base64_encode ( $apply_info ['id'] );
					return $data = array (
							'jumpurl' => $jumpurl,
							'flag' => 1 
					);
