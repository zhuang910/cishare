...
class Uploads extends CUCAS_Ext
{
	...
	function upload()
	{
		$path = UPLOADS . 'home/activity/' . date('Ym') . '/' . date('d') . '/';
		$path_web = UPLOADS_WEB . 'home/activity/' . date('Ym') . '/' . date('d') . '/';
		if (!is_dir($path)) {
			mk_dir($path);
		}
		$config['upload_path'] = $path;
		$config['allowed_types'] = 'gif|jpg|jpeg|png|doc|docx|pdf|xls|xlsx|mpg|wmv|avi|wma|mp3|mid|asf|rm|rmvb|wav|wma|mp4|swf';
	...
	function kindeditor()
	{
		$this->load->view('uploads/kindeditor');
	}
