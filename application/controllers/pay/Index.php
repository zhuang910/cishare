...
class Index extends Student_Basic {
	...
	function index() {
		...
		if ($applyid) {
			...
			// 首先查一下支付情况
			$apply_info_state = $this->apply_model->get_apply_info ( 'id = ' . $applyid . ' AND userid = ' . $_SESSION ['student'] ['userinfo'] ['id'] );
			// 已经支付成功的选手 直接跳到支付成功也去
			if ($apply_info_state ['paystate'] == 1) {
				
				$pay_success = '/' . $this->puri . '/student/apply/make_paymeznt?applyid=' . cucas_base64_encode ( $applyid );
...
// 			) );
			...
			//查看又没有生成收支表
			$budget_where="userid = {$_SESSION ['student']['userinfo'] ['id']} AND applyid = {$applyid}";
			$budget_result=$this->db->where($budget_where)->get('budget')->row_array();
			$budgetid=$budget_result['id'];
			if(empty($budget_result)){
				...
				$budgetid=$this->fee_model->insert_budget($budgetData);
				// 查询订单表 看是否有这条数据 支付的种类很多 因此 需要 添加支付类型 这个限制
				$where_order = "userid = {$_SESSION ['student']['userinfo'] ['id']} AND budget_id = {$budgetid} AND ordertype = 1";
				// 获取订单的信息
		...
		$this->load->view ( 'pay/index_index_test', array (
