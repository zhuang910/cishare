...
class Index extends Student_Basic {
	...
	function __construct() {
		...
		$this->load->vars ( 'scholarship_on', $this->scholarship_on );
		
		//判断  这个学生 是否是 在学
		$is_s = $this->db->select('*')->get_where('student','userid = '.$_SESSION['student']['userinfo']['id'])->result_array();
		if(!empty($is_s)){
			$this->is_student = 1;
		}
		$this->load->vars ( 'is_student', $this->is_student );
	}
	...
	function index() {
		...
		if($is_eav==2){
			...
			$this->load->view ( 'student/evaluate_index',array(
				'evaluate_time'=>$evaluate_time,
				'evaluate_info'=>$evaluate_info
				) );
		}else{
			$apply_unsub = array ();
			$where_apply = "userid = {$_SESSION['student']['userinfo']['id']}";
            ...
            if(!empty($apply_unsub)){
                foreach($apply_unsub as $k=>$v){
                    ...
                    if($v['state']==7||$v['state']==8){
                    	//判断是不是一年后
                    	if(time()-$v['applytime']<31536000000){
                       		 $is_apply_course=0;
                    	}
                    }
                }
            ...
            if(!empty($apply_unsub)){
            	foreach ($apply_unsub as $k => $v) {
            		$is_cause=0;
            		//查找订单
					$order_info=$this->db->get_where('apply_order_info','applyid = '.$v['id'].' AND ordertype = 5')->row_array();
					if(!empty($order_info)){
					...
					$apply_unsub[$k]['is_cause']=$is_cause;
            	}
            }
	...
	function apply_all() {
		// 查询未提交的申请
		$apply_unsub = array ();
		$where_apply = "userid = {$_SESSION['student']['userinfo']['id']} AND major.state < 1";
		$apply_unsub = $this->apply_model->get_apply_more_info ( $where_apply );
		$data = $this->returnData ( $where_apply );
		$apply_unsub = ! empty ( $data ['apply_info'] ) ? $data ['apply_info'] : array ();
		$course = ! empty ( $data ['course'] ) ? $data ['course'] : array ();
	...
	function del_apply() {
		...
		if ($id) {
			$where_apply = "userid = {$_SESSION['student']['userinfo']['id']} AND id = {$id} AND paystate != 1";
	...
	function evaluate(){
		...
		if(!empty($evaluate_time['starttime'])&&!empty($evaluate_time['endtime'])){
			if($now_time>$evaluate_time['starttime']&&$now_time<$evaluate_time['endtime']){
				...
				//没有课的情况呀
				$evaluate_info=$this->evaluate_model->get_course_teacher($_SESSION['student']['userinfo']['id']);
	...
	function evaluate_page(){
		...
		$this->load->view ( 'student/evaluate_page_index',array(
			...
			'item_info'=>$item_info,
			'majorid'=>!empty($majorid)?$majorid:0,
			'squadid'=>!empty($squadid)?$squadid:0,
