...
class Apply extends Student_Basic {
	...
	function index() {
		...
		if (! empty ( $courseid )) {
			...
			// 判断 信息用的
			$flag = $this->is_new_apply ( $courseid );
			if ($flag ['flag'] != 2) {
				echo '<script>window.location.href="' . $flag ['jumpurl'] . ' "</script>';
				die ();
			}
			// 查询数据
			$where_apply_info = "courseid = {$courseid} AND userid = {$_SESSION['student'] ['userinfo'] ['id']}";
			...
			$this->apply_history ( '开始申请', $apply_info ['id'] );
			...
			if (! empty ( $scholarship_on ) && $scholarship_on ['scholarship'] == 'yes') {
				...
				// 开启了 奖学金 设置
				/*
				 * if (! empty ( $coursename ['scholarship'] )) { $scholarship_major = $this->db->select ( '*' )->get_where ( 'scholarship_info', 'state = 1 AND id = ' . $coursename ['scholarship'] )->result_array (); // 已经申请的数量 且通过的 $count_apply = 0; // 数量 $count = $this->db->select ( 'id' )->get_where ( 'apply_info', 'scholorshipid = ' . $scholarship_major [0] ['id'] . ' AND state = 8' )->result_array (); if (! empty ( $count )) { $count_apply = count ( $count ); } if (empty ( $scholarship_major [0] ['count'] )) { $flag = 1; } else { if (($scholarship_major [0] ['count'] - $count_apply) > 0) { $flag = 1; $flag_count = $scholarship_major [0] ['count'] - $count_apply; } else { $flag_count = 0; } } }
				 */
				
				// 查询是否关联了 政府奖学金
				// 先判断 该专业 是否关联了 奖学金
				$scholarship_china = $this->db->select ( '*' )->limit ( 1 )->get_where ( 'major', 'id = ' . $courseid . ' AND scholarship IS NOT NULL' )->row ();
				if (! empty ( $scholarship_china )) {
					$scholarship = $this->db->select ( '*' )->get_where ( 'scholarship_info', 'apply_state = 2 AND state = 1 AND id IN (' . $scholarship_china->scholarship . ')' . ' AND ischinascholorship = 1' )->result_array ();
				}
			}
		}
		//获取专业有没有新生奖学金
		$major_info=$this->db->where('id',$courseid)->get('major')->row_array();
		$is_s=0;
		if(!empty($major_info['scholarship'])){
			$arr_d=explode(',', $major_info['scholarship']);
			foreach ($arr_d as $k => $v) {
				$deg_info=$this->db->where('id',$v)->get('scholarship_info')->row_array();
				if($deg_info['apply_state']==2&&$deg_info['ischinascholorship']!=1){
					$is_s=1;
				}
			}
		}
		$this->load->view ( 'student/apply_index', array (
				'courseid' => ! empty ( $courseid ) ? $courseid : '',
				'apply_info' => ! empty ( $apply_info ) ? $apply_info : array (),
				'issubmit' => $this->issubmit,
				'coursenames' => $coursename,
				'isstart' => $this->isstart,
				'scholarship_major' => ! empty ( $scholarship ) ? $scholarship : array (),
				'flag' => $flag,
...
/**
	...
	function upload_materials() {
		...
		$this->load->view ( 'student/apply_upload_materials', array (
	...
	function make_paymeznt() {
		$applyid = trim ( $this->input->get ( 'applyid' ) );
		if ($applyid) {
            $app = explode('-',cucas_base64_decode($applyid));
            $applyid = isset($app[0]) ? (int)$app[0] : cucas_base64_decode ($applyid);
			...
			// 查询课程的详情页
			$coursename = $this->course_model->get_one_content ( 'majorid = ' . $apply_info ['courseid'] );
			
			$this->issubmit = ! empty ( $apply_info ['issubmit'] ) ? $apply_info ['issubmit'] : 0;
			$this->isstart = ! empty ( $apply_info ['isstart'] ) ? $apply_info ['isstart'] : 0;
			$this->apply_history ( '点击支付', $applyid );
		}
		//查看凭据失败原因
		$is_cause=0;
		if($apply_info['paytype']==3){
			//查找订单
			$order_info=$this->db->get_where('apply_order_info','applyid = '.$apply_info['id'])->row_array();
			if(!empty($order_info)){
				$c_info=$this->db->get_where('credentials','orderid ='.$order_info['id'])->row_array();
				if($c_info['state']==2){
					$is_cause=$c_info['id'];
				}
			}
		...
		$this->load->view ( 'student/apply_make_payment', array (
				'courseid' => ! empty ( $apply_info ['courseid'] ) ? $apply_info ['courseid'] : '',
				'apply_info' => ! empty ( $apply_info ) ? $apply_info : array (),
				'course' => ! empty ( $course ) ? $course : array (),
				'issubmit' => $this->issubmit,
				'isstart' => $this->isstart,
				'coursename' => ! empty ( $course ) ? ( object ) $course : array (),
	...
	function submit_application() {
		...
		$this->load->view ( 'student/apply_submit_application', array (
				'courseid' => ! empty ( $apply_info ['courseid'] ) ? $apply_info ['courseid'] : '',
				'apply_info' => ! empty ( $apply_info ) ? $apply_info : array (),
				'course' => ! empty ( $course ) ? $course : array (),
				'issubmit' => $this->issubmit,
	 ...
	 * 判断信息
	 */
	private function is_new_apply($courseid = null) {
		if (! empty ( $courseid )) {
			...
			if ($flag == true) {
				...
				$data = array (
						...
						'agency_id'=>!empty($_SESSION['student']['userinfo']['agency_id'])?$_SESSION['student']['userinfo']['agency_id']:0,
						'is_agency'=>!empty($_SESSION['student']['userinfo']['agency_id'])?1:0,
	...
	/**
	 * [update_isatt更新申请表附件状态]
	 * @return [type] [description]
	 */
	function update_isatt(){
