 ...
 *        
 */
class Course extends Home_Basic {
	var $bread_line = null;
	/**
	 * 基础类构造函数
	 */
	...
	function index() {
		...
		$userid=intval(trim($this->input->get('userid')));
			...
			$course = $this->course_model->get_course_base ( $where );
			
			$course_name_all = $this->course_model->get_course_base ( 'state = 1' );
            ...
            //筛选出拒绝后的专业
            $not_course=$this->course_model->get_ont_course($userid);
            if(!empty($not_course)){
			...
			$this->load->view( '/master/agencyport/course_index', array (
	...
	function is_course_login() {
		...
		if (isset ( $userid )) {
			...
			if ($flag === true) {
				// 跳转到 申请页面
				ajaxReturn ( '/master/agencyport/apply?userid='.$userid.'&courseid=' . cucas_base64_encode ( $courseid ), '', 1 );
			} else {
				// 过期了 弹出 推荐页面
