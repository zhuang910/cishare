...
class FillingOutForms extends Student_Basic {
	...
	function apply() {
		...
		if (! empty ( $applyid )) {
			...
			//奖学金申请
			if(!empty($scholorshipid) && $issch == 1){
			...
			if ($cid) {
				...
				$this->load->view ( '/master/agencyport/fill_form', array (
		...
		if (IS_AJAX) {
			...
			if ($cid) {
				...
				if (! empty ( $is )) {
					...
					$url = $ischeck == 'ischeck' ? '/master/agencyport/apply/upload_materials?userid='.$data['userid'].'&applyid=' . $c_id : '';
					if ($ischeck) {
						...
						$this->apply_history ( '填写申请表', $cid,$data['userid'] );
	 ...
	 * 申请历史
	 */
	function apply_history($action = null, $applyid = null,$userid=null) {
		$data = array (
				'userid' => $userid,
				'appid' => $applyid,
				'action' => $action,
	 ...
	 * @param number $aid        	
	 */
	private function get_form_state($cid = 0,$userid) {
		$is = $this->model->get_apply_info ( $cid, $userid );
		if (! empty ( $is )) {
			/*
	 ...
	 * 验证课程是否可申请
	 */
	private function isapply($courseid ,$userid) {
