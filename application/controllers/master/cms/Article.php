...
class Article extends Master_Basic {
	...
	function index() {
		...
		if ($this->input->is_ajax_request () === true) {
			...
			if ($columnid == 15 || $columnid == 16) {
				$flag_p = $this->_get_power ();
				if (! $flag_p) {
					$where .= "groupid = " . $_SESSION ['master_user_info']->groupid;
				}
			}
			...
			foreach ( $output ['aaData'] as $item ) {
				...
				$item->operation = '<a title="编辑" class="blue" href="/master/cms/article/edit?columnid=' . $item->columnid . '&_id=' . $item->id . '">
		...
		$this->_view ( 'article_index', array (
				'columnid' => $columnid 
		) );
	}
