 ...
 *        
 */
class Majorimg extends Master_Basic {
	/**
	 * 专业图集
	 *
	 * @var array
	...
	function __construct() {
		...
		$this->load->model ( $this->view . 'majorimg_model' );
	}
	...
	function index() {
		...
		if ($this->input->is_ajax_request () === true) {
			...
			$output ['iTotalRecords'] = $output ['iTotalDisplayRecords'] = $this->majorimg_model->count ( $condition );
			$output ['aaData'] = $this->majorimg_model->get ( $fields, $condition );
			foreach ( $output ['aaData'] as $item ) {
				$item->operation = '
					<a href="/master/major/majorimg/edit?id=' . $item->id . '&label_id=' . $item->site_language . '" class="green"><i class="ace-icon fa fa-pencil bigger-130"></i></a>
		...
		$this->_view ( 'majorimg_index', array (
				'label_id' => $label_id,
				'majorid' => $majorid 
		) );
	}
	...
	function add() {
		...
		$this->_view ( 'majorimg_edit', array (
				'label_id' => $label_id,
				'majorid' => $majorid 
		) );
	}
	...
	function edit() {
		...
		if ($id) {
			$where = "id = " . $id;
			$info = ( object ) $this->majorimg_model->get_one ( $where );
			if (empty ( $info )) {
				$this->_alert ( '此文章不存在' );
			}
		}
	...
	function save() {
		...
		if (! empty ( $data )) {
			...
			if (! empty ( $id )) {
				$flag = $this->majorimg_model->save ( $id, $data );
			} else {
				$flag = $this->majorimg_model->save ( null, $data );
			}
	...
	function insert() {
		...
		if (! empty ( $data )) {
			...
			$id = $this->majorimg_model->save ( null, $data );
			if ($id) {
	...
	function update() {
		...
		if ($id) {
			...
			// 保存基本信息
			$this->majorimg_model->save ( $id, $data );
			ajaxReturn ( '', '更新成功', 1 );
		}
		ajaxReturn ( '', '更新失败', 0 );
	}
	...
	function del() {
		...
		if ($id) {
			$where = "id = {$id}";
			$info = ( object ) $this->majorimg_model->get_one ( $where );
			$is = $this->majorimg_model->delete ( $where );
			if ($is === true) {
				ajaxReturn ( '', '删除成功', 1 );
			}
		}
