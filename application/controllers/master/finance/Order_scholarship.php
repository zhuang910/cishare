 ...
 *        
 */
class Order_scholarship extends Master_Basic {
	/**
	 * 基础类构造函数
	 */
	...
	function index() {
		...
		if ($this->input->is_ajax_request () === true) {
			...
			foreach ( $output ['aaData'] as $item ) {
				...
				// var_dump($item->ordertype);
				$item->ordermondey = $this->order_model->get_money ( $item->applyid, $item->ordertype );
				// echo $this->db->last_query();exit;
				$item->ordertype=$this->order_model->get_ordertype($item->ordertype);
				// 
		...
		$this->_view ( 'order_scholarship_index', array (
				'ordername' => '奖学金订单',
				'label_id' => $label_id,
				'control' => 'order_scholarship' 
		) );
	}
