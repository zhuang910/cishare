...
class Card extends Master_Basic{
	//收费类型
	...
	function __construct() {
		...
		$this->load->model($this->view.'card_model');
	}
	...
	function index() {
		if ($this->input->is_ajax_request () === true) {
			...
			if(isset($sSearch_3) && $sSearch_3 !='') {
				$where .= " AND ( zust_slot_card.type LIKE '%{$sSearch_3}%')";
			}
			$sSearch_4 =  $_GET['sSearch_4' ];
			if (! empty ( $sSearch_4 )) {
				$where .= " AND ( zust_slot_card.number LIKE '%{$sSearch_4}%')";
			}
			$sSearch_5 =  $_GET['sSearch_5' ];
			if (! empty ( $sSearch_5 )) {
				$where .= " AND ( zust_slot_card.amount LIKE '%{$sSearch_5}%')";
			}
			$sSearch_6 =  $_GET['sSearch_6' ];
			if (! empty ( $sSearch_6 )) {
				$where .= " AND ( zust_slot_card.id LIKE '%{$sSearch_6}%')";
			}

			// 输出
			$output ['sEcho'] = intval ( $_GET ['sEcho'] );
			$output ['iTotalRecords'] = $output ['iTotalDisplayRecords'] = $this->card_model->count ( $where);
			$output ['aaData'] = $this->card_model->get ( $where, $limit, $offset, $orderby );
			foreach ( $output ['aaData'] as $val ) {
				...
				$val->field_10 = '
								<a class="green" title="编辑修改备注" href="javascript:;" onclick="pub_alert_html(\'' . $this->zjjp . '/card/outline?id=' . $val->id . '&s=1\')"><i class="ace-icon fa fa-leaf bigger-130"></i></a><br />
								<span style="color:#F00;"><a href="javascript:pub_alert_html(\'card/editproof?id=' . $val->id . '\');">查看凭据</a></span>
								';
	...
	function outline(){
		...
		$remark=$this->card_model->get_student_remark($id);
