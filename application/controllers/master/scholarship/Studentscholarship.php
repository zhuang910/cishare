 ...
 *        
 */
class Studentscholarship extends Master_Basic {
...
// 		}
		$where = 'applyscholarship_info.type = 1 AND applyscholarship_info.state = ' . $label_id . '';
		// 根据状态获得申请列表信息
	  ...
	  $major_info = $this->student_model->get_major_info ('id>0',0,0,'language desc');
		...
		//获取 所有的在学奖学金
		$s_a = $this->db->select('id,title')->get_where('scholarship_info','id > 0 AND state = 1 AND apply_state = 1')->result_array();
		$data = array (
				'label_id' => $label_id,
				'lists' => $lists,
				'app_state' => $app_state,
		...
		$this->load->view ( 'master/scholarship/studentscholarship_index', $data );
	}
