 ...
 *        
 */
class Appmanager extends Master_Basic {
	...
	function __construct() {
		...
		//查 所有的 奖学金申请 且 通过的
		$zyjdata = array();
		$sql = 'SELECT `scholorshipid`, count(scholorshipid) AS count FROM (`zust_apply_info`) WHERE `state` = 8 GROUP BY `scholorshipid`';
		$result = $this->db->query($sql)->result_array();
		if(!empty($result)){
			foreach ($result as $zyjk => $zyjv){
				$zyjdata[$zyjv['scholorshipid']] = $zyjv['count'];
			}
		}
		$this->load->vars('zyjdata',$zyjdata);
	...
	function index() {
		...
		if ($label_id == 1) {
			$where = 'apply_info.state = 1 OR apply_info.state=3';
		} else {
			$where = 'apply_info.state = ' . $label_id . '';
		}
		if($_SESSION['master_user_info']->groupid==9){
			//获取学院的信息
			$faculty_info=$this->db->where('userid = '.$_SESSION['master_user_info']->id)->get('faculty')->row_array();
			//获取该学院的所有的专业
			$major_info=$this->db->where('facultyid = '.$faculty_info['id'])->get('major')->result_array();
			$major_str='';
			if(!empty($major_info)){
			...
			$where='courseid IN ('.trim($major_str,',').')';
			$where.=' AND faculty_state = '.$label_id;
			$where.=' AND apply_info.is_faculty =1';
		}
		// 根据状态获得申请列表信息
        ...
        // 操作老师
        $admin_lists = $this->_get_admin_user();
		...
		//中介搞成数组
		$agency_info=$this->db->get_where('agency_info')->result_array();
		$arr=array();
		foreach ($agency_info as $key => $value) {
			$arr[$value['id']]=$value['company'];
		}
		$data = array (
				'label_id' => $label_id,
				'lists' => $lists,
		...
		// 此处用户处理交押金问题中押金是否交付
// 		if ($label_id == 6) {
// 			$this->load->view ( 'master/enrollment/appmanager/app_index_1', $data );
// 		} else {
// 			$this->load->view ( 'master/enrollment/appmanager/app_index', $data );
// 		}
		$this->load->view ( 'master/enrollment/appmanager/app_index', $data );
	}
	/**
...
// 		}
	...
	function app_finish() {
		...
		foreach ( $nationality['global_country_cn'] as $key => $value ) {
			$country [$key] = $value;
		}
	...
	function proof() {
		...
		if(!empty($proof_all)){
			foreach ($proof_all as $key => $value) {
				if(!empty($value['budgetid'])){
					$budget=$this->db->get_where('budget','id = '.$value['budgetid'])->row_array();
					if(!empty($budget)){
						$is=$this->db->get_where('apply_info','id = '.$budget['applyid'])->row_array();
						if(!empty($is)){
							continue;
						}
					}
				...
				if(!empty($value['orderid'])){
					$order=$this->db->get_where('apply_order_info','id = '.$value['orderid'])->row_array();
					if(!empty($order)){
						$budget=$this->db->get_where('budget','id = '.$order['budget_id'])->row_array();
						if(!empty($budget)){
							$is=$this->db->get_where('apply_info','id = '.$order['applyid'])->row_array();
							if(!empty($is)){
								continue;
							}
						}
		...
		if (! empty ( $proof_all )) {
			...
			// 订单信息
			$where_order = 'id IN (' . trim(implode ( ',', $orderid ),',') . ')';
	...
	function lookproof(){
		$id = $this->input->get ( 'id' );
		$look_data = $this->app->get_lookproof_data($id);
		$nationality = $this->run_unit_nationality;//国籍
		foreach ($nationality['global_country_cn'] as $k => $v) {
			if($look_data['remit_nationality'] == $k){
				$look_data['remit_nationality'] = $v;
			}
		}
		$html = $this->load->view('master/enrollment/appmanager/lookproof.php',array(
			'look_data' => $look_data
			),true);
		ajaxReturn ( $html, '', 1 );
	}
	function xiangxi(){
    	...
    	$html = $this->load->view('master/enrollment/appmanager/remark', array(
    			'id'=>$id,
    			'orderid'=>$orderid,
    			'applyid'=>$applyid,
    			'state'=>$state,
	...
	function goproof() {
		...
		if (! empty ( $id ) && ! empty ( $orderid ) && ! empty ( $applyid ) && ! empty ( $state ) && ! empty ( $userid )) {
			// 订单信息
			$o = $this->db->select ( '*' )->get_where ( 'apply_order_info', 'id = ' . $orderid )->row ();
	...
	function xz() {
		...
		if (! empty ( $id ) && ! empty ( $courseid )) {
			...
			// xmlns:w="urn:schemas-microsoft-com:office:word"
				//申请表iD
			$applyinfo=$this->db->get_where('apply_info','id = '.$id)->row_array();
			$applytemplate=$this->db->get_where('major','id = '.$applyinfo['courseid'])->row_array();

			if($applytemplate['applytemplate']==6895){
				// xmlns="http://www.w3.org/TR/REC-html40">';
				$html = $this->load->view ( 'master/enrollment/appmanager/app_short_form', array (
						'userpage' => $userpage,
						'edu' => $edu,
						'family' => $family,
			...
			}elseif ($applytemplate['applytemplate']==6874) {
				// xmlns="http://www.w3.org/TR/REC-html40">';
				$html = $this->load->view ( 'master/enrollment/appmanager/appform', array (
						'userpage' => $userpage,
						'edu' => $edu,
						'family' => $family,
						'PreviousEducation' => $PreviousEducation,
...
// 			$file_name .= !empty($email_w->email)?'_'.$email_w->email:'';
	...
	function browse() {
		...
		if (! empty ( $id ) && ! empty ( $courseid )) {
			...
			*/
			...
			if($applytemplate['applytemplate']==6895){
				$this->load->view ( 'master/enrollment/appmanager/app_short_form', array (
					'userpage' => $userpage,
					'edu' => $edu,
					'family' => $family,
					'PreviousEducation' => $PreviousEducation,
				...
				$this->load->view ( 'master/enrollment/appmanager/appform', array (
					'userpage' => $userpage,
					'edu' => $edu,
					'family' => $family,
					'PreviousEducation' => $PreviousEducation,
	 ...
	 * 设置学费折扣
	 */
	function edit_is_tuition_discount(){
		$data=$this->input->post();
		if(!empty($data['value'])){
			if(!is_numeric($data['value'])){
				ajaxReturn('','您输入的不是数字',0);
			}
			$this->db->update('apply_info',array('tuition_discount'=>$data['value']),'id = '.$data['pk']);
			//申请信息
			$apply_info=$this->db->get_where('apply_info','id = '.$data['pk'])->row_array();
			//插入所有收支表  
			//先查询有没有插入收支表
			$result=$this->db->get_where('budget','applyid = '.$data['pk'].' AND type = 15')->row_array();
			if(!empty($result)){
				//更新收支表
				$this->db->update('budget',array('payable'=>$data['value']),'applyid = '.$data['pk'].' AND type = 15');
				//更新申请减免学费表
				$this->db->update('apply_remission_tuition',array('pay'=>$data['value']),'applyid = '.$data['pk']);
			}else{
				//组合收支表的字段
				$budget=array(
				...
				$tui_budget=array(
						...
						'budget_type'=>2,
						'type'=>6,
						'should_returned'=>$data['value'],
						'true_returned'=>$data['value'],
						'returned_time'=>time(),
						'createtime'=>time(),
						'applyid'=>$data['pk'],
						'adminid'=>$_SESSION['master_user_info']->id
					);
				$this->fee_model->insert_budget($tui_budget);
				//插入学费表
				$tuition=array(
					'budgetid'=>$budgetid,
					'nowterm'=>1,
	...
	/**
	 * [faculty_change_state 二级学院改状态弹框]
	 * @return [type] [description]
	 */
	function faculty_change_state_page(){
		$appid=$this->input->get('id');
		$info=$this->db->get_where('apply_info','id = '.$appid)->row_array();
		$html = $this->_view ( 'faculty_change_state_page', array (
				'appid'=>$appid,
				'info'=>$info
				), true );
			ajaxReturn ( $html, '', 1 );
	...
	/**
	 * [faculty_change_state 修改二级学院状态]
	 * @return [type] [description]
	 */
	function faculty_change_state(){
		$data=$this->input->post();

		$id=$data['id'];
		unset($data['id']);

		if($data['faculty_state']==0){
			ajaxReturn('','请选择申请状态',0);
		}
		$data['faculty_time']=time();
		$data['faculty_adminid']=$_SESSION['master_user_info']->id;
		$this->db->update('apply_info',$data,'id = '.$id);
		ajaxReturn('','修改成功',1);
	}
	/**
	 * [look_faculty_remark 插卡安二级学院备注]
	 * @return [type] [description]
	 */
	function look_faculty_remark(){
		$appid=$this->input->get('id');
		$info=$this->db->where('id ='.$appid)->get('apply_info')->row_array();
		$html = $this->_view ( 'look_faculty_remark', array (
			'info'=>$info
				), true );
			ajaxReturn ( $html, '', 1 );
	}
	...
	function moban(){
		...
		$html = $this->load->view('master/enrollment/appmanager/print_select',array(
               'info'=>$info,
               'applyid'=>$applyid
            ),true);
        ajaxReturn($html,'',1);
	...
	function export_by(){
			$this->load->library ( 'sdyinc_export' );
			$data=$this->_get_ex_by_tou_data();
			$body_data=$this->_get_ex_by_body_data();
			$d = $this->sdyinc_export->do_export_by ($data,$body_data);
			if (! empty ( $d )) {
				$this->load->helper ( 'download' );
				force_download ( 'by' . time () . '.xlsx', $d );
				return 1;
			}
	}
	/**
	 * [_get_ex_by_data 导出北邮报表头部]
	 * @return [type] [description]
	 */
	 function _get_ex_by_tou_data(){
	 	$data[]='ID';
	 	$data[]='申请编号';
		$data[]='学号';
		$data[]='护照姓名';
	 ...
	 /**
	  * [_get_ex_by_body_data 获取数据]
	  * @return [type] [description]
	  */
	 function _get_ex_by_body_data(){
	 	//查询所有的申请
	 	$apply_info=$this->db->get_where('apply_info')->result_array();
	 	$body_data=array();
	 	//国籍
	 	$nationality=CF('public','',CACHE_PATH);
	 	if(!empty($apply_info)){
	 		foreach ($apply_info as $k => $v) {
	 			$userinfo=$this->db->get_where('student_info','id = '.$v['userid'])->row_array();
	 			$major_info=$this->db->get_where('major','id = '.$v['courseid'])->row_array();
	 			$degree_info=$this->db->get_where('degree_info','id = '.$major_info['degree'])->row_array();
	 			$body_data[$k][]=!empty($v['id'])?$v['id']:'';
			 	$body_data[$k][]=!empty($v['number'])?$v['number']:'';
				$body_data[$k][]=!empty($userinfo['studentid'])?$userinfo['studentid']:'';
				$body_data[$k][]=!empty($userinfo['firstname'])||!empty($userinfo['lastname'])?$userinfo['firstname'].' '.$userinfo['lastname']:'';
				$body_data[$k][]=!empty($userinfo['chname'])?$userinfo['chname']:'';
				$body_data[$k][]=!empty($userinfo['birthday'])?date('Y-m-d',$userinfo['birthday']):'';
				$body_data[$k][]=!empty($userinfo['birthplace'])?$userinfo['birthplace']:'';
				$faculty=$this->_get_faculty($v['courseid']);
				$body_data[$k][]=$faculty;
				$body_data[$k][]=!empty($major_info['name'])?$major_info['name']:'';
				$body_data[$k][]=!empty($major_info['name'])?$major_info['name']:'';
				$body_data[$k][]=!empty($userinfo['nationality'])?$nationality['global_country_cn'][$userinfo['nationality']]:'';
				$body_data[$k][]=!empty($degree_info['title'])?$degree_info['title']:'';
				$body_data[$k][]=$degree_info['isdegree']==1?'长期':'短期';
				$body_data[$k][]=!empty($userinfo['sex'])?($userinfo['sex']==1?'男':'女'):'';
				$jingfei=$this->_get_jingfei($v['userid']);
				$body_data[$k][]=$jingfei;
				$body_data[$k][]=!empty($major_info['name'])?$major_info['name']:'';
				$body_data[$k][]=$jingfei;
				$body_data[$k][]=!empty($userinfo['marital'])?($userinfo['marital']==1?'是':'否'):'';
				$body_data[$k][]='CSC编号';
				$body_data[$k][]=!empty($userinfo['birthday'])?$userinfo['birthday']:'';
				//判断申请录取时间
				
				$body_data[$k][]=$v['state']==7||$v['state']==8?'录取':'申请';
				$body_data[$k][]=!empty($v['applytime'])?date('Y-m-d',$v['applytime']):'';
				$body_data[$k][]=!empty($v['lasttime'])?date('Y-m-d',$v['lasttime']):'';
				$body_data[$k][]=$faculty;
				$body_data[$k][]=$faculty;
				$body_data[$k][]=!empty($major_info['opentime'])?date('Y-m-d',$major_info['opentime']):'';
				$body_data[$k][]=!empty($major_info['endtime'])?date('Y-m-d',$major_info['endtime']):'';
				$body_data[$k][]=!empty($v['remark'])?$v['remark']:'';
				$body_data[$k][]=!empty($major_info['language'])?($major_info['language']==1?'中文':'英文'):'';
	  ...
	  * @return [type] [description]
	  */
	 function _get_faculty($majorid){
	 	if(!empty($majorid)){
	 		$major_info=$this->db->get_where('major','id = '.$majorid)->row_array();
	 		if(!empty($major_info)){
	 			$data=$this->db->get_where('faculty','id = '.$major_info['facultyid'])->row_array();
	 			return $data['name'];
	 		}
	 	}
	 	return '';
	 ...
	 /**
	  * [_get_jingfei 获取学生的经费]
	  * @return [type] [description]
	  */
	 function _get_jingfei($userid){
	 	if(!empty($userid)){
	 		//查询奖学金申请表有没有申请成功的
	 		$app_sc=$this->db->get_where('applyscholarship_info','state = 5')->row_array();
	 		if(!empty($app_sc)){
	 			//获取奖学金的信息
	 			$sc_info=$this->db->get_where('scholarship_info','id ='.$app_sc['scholarshipid'])->row_array();
	 			return $sc_info['title'];
	 		}
	 	}
	 	return '个人经费';
	 }
	  ...
	  * @return [type] [description]
	  */
	 function erjixueyuan(){
	 	$id=$this->input->get('id');
	 	if(!empty($id)){
	 		$this->db->update('apply_info',array('is_faculty'=>1),'id='.$id);
	 		ajaxReturn('','',1);
	 	}
	 	ajaxReturn('','',0);
	 }

    private function _get_admin_user(){
        $this->load->model('master/core/user_model');
        $lists = $this->user_model->get();
        $temp = array();
        foreach ($lists as $item) {
            $temp[$item->id]['username'] = $item->username;
        }
	...
	//初始化
	function addproofremark() {
		$id = intval ( $this->input->get ( 'id' ) );
		$result = $this->db->select('id,remark')->get_where('credentials','id = '.$id)->row();
		$html = $this->load->view ( 'master/enrollment/appmanager/addproofremark', array(
				'result' => $result
		), true );
		ajaxReturn ( $html, '', 1 );
	...
	function save_proofremark() {
		...
		if(!empty($value)){
			...
			$flag = $this->db->update('credentials',array('remark' => $value),'id = '.$id);
			if($flag){
				ajaxReturn('','',1);
