...
<SCRIPT type="text/javascript">
	function do_authority_save(){
		...
		$.ajax({
			type:'post',
			url:'/admin/authority/group/savepower',
	...
	var zNodes =[
		...
		{id:22,pId:2,name:'权限分配',zjj_v:'<?=md5('/admin/authority/group')?>'<?=!empty($power) && in_array(md5('/admin/authority/group'),$power) ? ',checked:true':''?>},
		{id:23,pId:2,name:'日志管理',zjj_v:'<?=md5('/admin/authority/log')?>'<?=!empty($power) && in_array(md5('/admin/authority/log'),$power) ? ',checked:true':''?>},
		{id:211,pId:21,name:'管理员帐号管理',zjj_v:'<?=md5('/admin/authority/admin')?>'<?=!empty($power) && in_array(md5('/admin/authority/admin'),$power) ? ',checked:true':''?>},

		{ id:51,pId:5,name:'文章管理',zjj_v:'<?=md5('/admin/article')?>'<?=!empty($power) && in_array(md5('/admin/article'),$power) ? ',checked:true':''?>},
		{ id:51,pId:5,name:'分类管理',zjj_v:'<?=md5('/admin/article/category')?>'<?=!empty($power) && in_array(md5('/admin/article/category'),$power) ? ',checked:true':''?>},
		{ id:52,pId:5,name:'专题管理',zjj_v:'<?=md5('/admin/article/special')?>'<?=!empty($power) && in_array(md5('/admin/article/special'),$power) ? ',checked:true':''?>},
		{ id:51,pId:5,name:'评论管理',zjj_v:'<?=md5('/admin/article/reply')?>'<?=!empty($power) && in_array(md5('/admin/article/reply'),$power) ? ',checked:true':''?>},
