...
<li class="<?=!empty($uri3) &&$uri4!='part_class' && in_array($uri3, array('article','category','special','reply'))?'active open':''?>  hsub">
	...
	<ul class="submenu nav-hide" class="submenu nav-hide" <?=!empty($uri3) && in_array($uri3, array('article','category','special','reply'))?'style="display: block;"':''?>>
		<?php if(!empty($_SESSION['power']) && in_array(md5('/admin/article'), $_SESSION['power'])){?>
			<li class="<?=$uri3 == 'article'?'active':''?>">
				<a href="/admin/article/article">
		...
		<?php if(!empty($_SESSION['power']) && in_array(md5('/admin/article/category'), $_SESSION['power'])){?>
		...
		<?php if(!empty($_SESSION['power']) && in_array(md5('/admin/article/special'), $_SESSION['power'])){?>
			<li class="<?=$uri3 == 'special'?'active':''?>">
				<a href="/admin/article/special">
					<i class="menu-icon fa fa-caret-right"></i>
					专题管理
				</a>
			</li>
		...
		<?php if(!empty($_SESSION['power']) && in_array(md5('/admin/article/reply'), $_SESSION['power'])){?>
			<li class="<?=$uri3 == 'reply'?'active':''?>">
				<a href="/admin/article/reply">
					<i class="menu-icon fa fa-caret-right"></i>
					评论管理
				</a>
			</li>
