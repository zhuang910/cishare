	...
	<div class="applyonline-4-main">
		...
		<div class="applyonline-4-total">
			<dl>
				...
				<dd><span class="width-4-1"><em class="color-blue"><a href="/<?=$puri?>/course/detail?id=<?=$course['id']?>" target="_blank"><?php if($puri == 'en'){?><?=!empty($coursename->englishname)?$coursename->englishname:''?><?php }else{?><?=!empty($coursename->name)?$coursename->name:''?><?php }?></a></em></span><span class="width-4-2"><strong><?php if(!empty($apply_info['danwei']) && $apply_info['danwei'] == 1){echo 'USD';}else{echo 'RMB';}?> <?php if($isapplypay == 1){?><?=!empty($apply_info['registration_fee'])?$apply_info['registration_fee']:''?><?php }else{?>0<?php }?></strong></span><span class="width-4-3"><strong><?=lang('paid'.$apply_info['paystate'])?>
					<br />
					<?php if($is_cause!=0){?>
	                 <!--凭据失败原因-->
	                 <?=lang('pujushifuzhibai')?><br />
	                 <a style="color:blue" href="javascript:;" onclick="look_cause(<?=$is_cause?>)"><?=lang('xiangxi')?></a>
	                 <?php }?>
				</strong></span></dd>
			</dl>
			...
			<div class="applyonline-2-btn">
				<div class="applyonline-2-btn-1 f_l"><a href="/<?=$puri?>/student/apply/upload_materials?applyid=<?=!empty($apply_info['id'])?cucas_base64_encode($apply_info['id']):''?>"><?=lang('apply_prev')?></a></div>
				<div class="redbtn">
				<?php if($apply_info['paystate'] == 1){?>
...
<script type="text/javascript">
	<!--支付成功 直接到下一步去-->
	function go_next(){
		window.location.href='/<?=$puri?>/student/apply/submit_application?applyid=<?=!empty($apply_info['id'])?cucas_base64_encode($apply_info['id']):''?>';
	}
	<!--点击下一步 跳到支付去-->
	function pay_next(){
	   var url = '/<?=$puri?>/pay?applyid='+'<?=!empty($apply_info['id'])?cucas_base64_encode($apply_info['id']):''?>';
	   window.open(url);
	   var d = dialog({
			id:'cucasdialog',
