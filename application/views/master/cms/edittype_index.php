	...
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
				...
				<div class="table-header">
					<?=!empty($name)?$name:''?>列表
					<button style="float:right;" onclick="add()" class="btn btn-primary btn-sm btn-default btn-sm" title="添加<?=!empty($name)?$name:''?>" type="button">
...
function add(){
		window.location.href="<?=$zjjp?>type/edit_type?parent_id=<?=$parent_id?>";
	}
	...
	$('#sample-table-2').each(function(){
		...
		if($(this).hasClass("dataTable-ajax")){
			...
			opt.sAjaxSource = "<?=$zjjp?>type/edittype?parent_id=<?=$parent_id?>";
		}
...
function del(id){
	pub_alert_confirm('/master/cms/type/del_type?id='+id);
