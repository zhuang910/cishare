...
<li class="<?=!empty($uri3) && $uri2=='authority'&&in_array($uri3, array('admin','teacher','agency','group','log','society'))?'active open':''?>  hsub">
	...
	<?php if(!empty($_SESSION['power']) && in_array(md5('col_username'), $_SESSION['power'])){?>
		<li class="<?=!empty($uri3) && in_array($uri3, array('admin','teacher','agency','society'))?'active open':''?>  hsub">
			...
			<?php if(!empty($_SESSION['power']) && in_array(md5('/master/authority/admin'), $_SESSION['power'])){?>
				<li class="<?=$uri3 == 'admin'?'active':''?>">
					<a href="/master/authority/admin/index"><i class="menu-icon fa fa-caret-right"></i>
						管理员帐号
					</a>
				</li>
			<?php }?>
		...
		<?php }?>
		<?php if(!empty($_SESSION['power']) && in_array(md5('/master/authority/group'), $_SESSION['power'])){?>
		<li class="<?=$uri3 == 'group'?'active':''?>">
			<a href="/master/authority/group"><i class="menu-icon fa fa-caret-right"></i>
				权限管理
			</a>
		</li>
		<?php }?>
		<?php if(!empty($_SESSION['power']) && in_array(md5('/master/authority/log'), $_SESSION['power'])){?>
		<li class="<?=$uri3 == 'log'?'active':''?>">
			<a href="/master/authority/log"><i class="menu-icon fa fa-caret-right"></i>
				日志管理
			</a>
		</li>
	<?php }?>
