...
<div class="row">
	<div class="col-sm-12">
		...
		<div>
			...
			其他新生奖学金管理
				<a type="button" title="人工授予奖学金" href="/master/scholarship/otherscholarship/add_otherscholarship?type=2" class="btn btn-primary btn-sm btn-default btn-sm" style="float:right;">
			...
			<ul class="nav nav-tabs" style="padding-top:3px;padding-left:5px;">
                    ...
                    <a href="/master/scholarship/otherscholarship/index?&label_id=0"><h5>未完成</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='1'):?> class="active"<?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?label_id=1"><h5>处理中</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='2'):?> class="active"<?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?&label_id=2"><h5>打回</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='3'):?> class="active" <?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?&label_id=3"><h5>打回提交</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='4'):?> class="active"<?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?&label_id=4"><h5>拒绝</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='5'):?> class="active"<?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?&label_id=5"><h5>通过</h5></a>
                    </li>
					<li <?php if(!empty($label_id) && $label_id =='6'):?> class="active"<?php endif;?>>
                    <a href="/master/scholarship/otherscholarship/index?&label_id=6"><h5>结束</h5></a>
                    </li>
			...
			<div>
				<table class="table table-hover table-nomargin dataTable dataTable-for-templates table-bordered">
						...
						<?php  if(!empty($lists)):?>
                            ...
                            <tr id="<?=$val->appid; ?>">
                                ...
                                <td>
									...
									学期:<?=!empty($val->term)?'第'.$val->term.'学期':''?>
                                  费用类型:<br />
									<?php 
					...
					<td>
						...
						<?php if($val->is_artificial_grant==0){?>
							申请表： 
							<a href="/master/scholarship/otherscholarship/xz?id=<?=$val->appid?>&courseid=<?=$val->scholarshipid?>&userid=<?=$val->userid?>&type=online">
																							   下载</a>
							<a href="/master/scholarship/otherscholarship/browse?id=<?=$val->appid?>&courseid=<?=$val->scholarshipid?>&userid=<?=$val->userid?>&type=online" target="_blank">
							预览</a><br/>
													   附件：
						  
							<a href="javascript:pub_alert_html('/master/scholarship/otherscholarship/check_upload?id=<?=$val->appid?>&courseid=<?=$val->scholarshipid?>');"  rel="tooltip" data-original-title="审核">审核</a>
					
							<a href="/master/scholarship/otherscholarship/attach_download?id=<?=$val->appid;?>">                           
													   下载</a>
						<?php }else{?>
							<span class="label label-success">人工授予奖学金</span>
						<?php }?>
					...
					<div class="btn-group">
						<a id="update" data-type="textarea"  class="btn btn-xs btn-info" data-placement="left"  data-value='<?=$val->remark; ?>' data-pk="<?=$val->appid; ?>^text" href="javascript:pub_alert_html('/master/scholarship/change_scholarship_status/update_jiangxuejin_page?id=<?=$val->appid?>&label_id=<?=$label_id?>');" rel="tooltip" title="更换奖学金">
							 更换奖学金
						  </a>  
						<button data-toggle="dropdown" class="btn btn-xs btn-info btn-white dropdown-toggle">
								...
								<ul class="dropdown-menu dropdown-info dropdown-menu-right">
									...
									<?php if ($val->status != 6) :?>
										<li> <a href="javascript:pub_alert_html('/master/scholarship/change_scholarship_status/index?id=<?=$val->appid?>&label_id=<?=$label_id?>');">修改状态</a></li>
									<?php endif;?>
