									...
									<div class="modal-content">
										...
										<div>
											...
											<input type="hidden" value="<?=$studentid?>" name="studentid">
												日期:<input onclick="biao()" name="date" onchange="quick_changedate()" id="starttime" style="width:100px;"  class="date-picker valid" type="text" data-date-format="yyyy-mm-dd" value="">
												...
												<label class="control-label" for="platform">类别:</label>
...
function insert_checking(){
	var data=$('#checking').serialize();
		...
		$.ajax({
			url: "<?=$zjjp?>teacher_checking/insert_checking",
			type: 'POST',
			dataType: 'json',
			data: data,
		})
...
function quick_changedate(){
	...
	$.ajax({
		url: "<?=$zjjp?>teacher_checking/get_checkeing_course?date="+date+"&classtime="+<?=$classtime['classtime']?>+'&studentid='+<?=$studentid?>+'&squadid='+<?=$squadid?>+'&majorid='+<?=$majorid?>,
		type: 'POST',
		dataType: 'json',
		data: {},
	})
...
function luru(id){
	...
	var data=$('#'+id).serialize();
		...
		$.ajax({
			url: "<?=$zjjp?>teacher_checking/insert?time="+time,
			type: 'POST',
			dataType: 'json',
			data: data,
		})
...
function shuaxin(mid,sid){
	$.ajax({
		url: '<?=$zjjp?>teacher_checking/qin?mid='+mid+'&sid='+sid,
		type: 'POST',
		dataType: 'json',
		data: {},
	})
