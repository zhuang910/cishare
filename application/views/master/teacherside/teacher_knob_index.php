	...
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
									...
									<div class="table-header">
										课程列表
										<a style="float:right;" onclick="pub_alert_html('<?=$zjjp?>teacher_knob/add_course?teacherid=<?=$teacherid?>&s=1')" class="btn btn-primary btn-sm btn-default btn-sm" title="关联课程" type="button">
...
function add(){
	window.location.href='/master/teacherside/teacher_knob/add?teacherid='+<?=$teacherid?>;
}
	...
	$('#sample-table-2').each(function(){
		var opt = {
			...
			"oLanguage":{
				...
				"sInfoEmpty" : "没有记录",
				"sInfoFiltered" : "",
				"sZeroRecords" : "<a href='<?=$zjjp?>teacher_knob/add?teacherid="+<?=$teacherid?>+"'><span>为老师添加课程吧</span></a>",
			}
		};
		...
		if($(this).hasClass("dataTable-ajax")){
			...
			opt.sAjaxSource = "<?=$zjjp?>teacher_knob?teacherid="+<?=$teacherid?>;
		}
...
function del(id){
	pub_alert_confirm('/master/teacherside/teacher_knob/del?id='+id);
}
</script>
