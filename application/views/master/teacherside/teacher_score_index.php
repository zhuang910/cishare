									...
									<div class="form-group">
										...
										<div class="col-xs-12 col-sm-5">
											<select id="course" class="col-sm-8" name="courseid" aria-required="true" aria-invalid="false" onchange="c()">
												<option value="0">—请选择—</option>
												<?php foreach($t_course as $k=>$v):?>
													<option value="<?=$v['courseid']?>"><?=$v['name']?></option>
												<?php endforeach;?>
											</select>
										</div>
									</div>
...
$("#nowterm").empty();
	var mid=$('#majorid').val();
		$.ajax({
			url: '<?=$zjjp?>teacher_score/get_nowterm/'+mid,
			type: 'POST',
			dataType: 'json',
			data:{},
		})
...
function term(){
	...
	var mid=$('#majorid').val();
		...
		$.ajax({
			url: '<?=$zjjp?>teacher_score/get_squad?term='+term+'&mid='+mid,
			type: 'POST',
			dataType: 'json',
			data:{},
		})
...
function sure(){
					...
					$.ajax({
						url: '<?=$zjjp?>teacher_score/get_student',
						type: 'POST',
						dataType: 'json',
						data: data,
					})
					.done(function(r) {
						if(r.state==1){
							...
							$.each(r.data.stu, function(k, v) {
								...
								strr='<tr><td width="200">'+v.studentid+'</td><td width="200">'+v.name+'</td><td>'+v.enname+'</td><td><input name="'+v.id+'" type="text" data-score="true"></td></tr>';
								$.each(r.data.scoreinfo, function(kk, vv) {
									 if(v.id==vv.studentid&&vv.courseid==cid&&vv.term==term&&vv.squadid==sid&&vv.scoretype==type){
									 	strr='<tr><td width="200">'+v.studentid+'</td><td width="200">'+v.name+'</td><td>'+v.enname+'</td><td>'+vv.score+'<a class="red" onclick="del('+vv.id+')" href="javascript:;"><i class="ace-icon fa fa-trash-o bigger-130"></i></a></td></tr>';
									 }
								});
								str+=strr;
							}); 
...
function score_submit(){
	...
	$.ajax({
		url: '<?=$zjjp?>teacher_score/save_score?'+dataone,
		type: 'POST',
		dataType: 'json',
		data: datatwo,
	})
            ...
            callback: function(result) {
              ...
              if(result){
				...
				var value=$('#value').val();
                  $.ajax({
                    type:'POST',
                    url:'<?=$zjjp?>teacher_score/del?id='+id+'&sid='+sid+'&key='+key+'&value='+value,
                    data:{},
                    dataType:'json',
                    success:function(r){
                       	if(r.state==1){
