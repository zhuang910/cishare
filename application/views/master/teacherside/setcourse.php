		...
		<div class="modal-content">
			<div class="modal-header no-padding">
				<div class="table-header">
					<button type="button" class="close" onclick="refresh()" data-dismiss="modal" aria-hidden="true">
										...
										<td width='298'>
											<?php if($cid1!=0):?>
												<form class="form-horizontal" id="setcourse<?=!empty($courseinfo[$i]['id'])?$courseinfo[$i]['id']:''?>" method="post" action="" >
													...
													<input type="hidden" value="99" name="week">
													<input type="hidden" value="99" name="knob">
										...
										<td width='298'>
											<?php if($cid2!=0):?>
												<form class="form-horizontal" id="setcourse<?=!empty($courseinfo[$i+1]['id'])?$courseinfo[$i+1]['id']:''?>" method="post" action="" >
													...
													<label class="inline">
														<input <?=$checked2?> class="ace" onchange="setcourse(<?=!empty($courseinfo[$i+1]['id'])?$courseinfo[$i+1]['id']:''?>)" value="<?=!empty($courseinfo[$i+1]['id'])?$courseinfo[$i+1]['id']:''?>" type="checkbox" name="courseid">
...
function up(page){
	if(page==0){
		...
		$.ajax({
			url: "<?=$zjjp?>teacher_knob/up_course?page="+page+"&teacherid="+<?=$teacherid?>,
			type: 'POST',
			dataType: 'json',
			data: data,
		})
			...
			if(r.state==1){
				...
				$.each(r.data.c, function(i, v) {
					...
					if((i)%2==0){
						...
						str+='<input type="hidden" value="99" name="week"><input type="hidden" value="99" name="knob">';
						str+='<input type="hidden" value="<?=$teacherid?>" name="teacherid" />';
...
function next(page){
		...
		var data=$('#search').serialize();
		$.ajax({
			url: "<?=$zjjp?>teacher_knob/next_course?page="+page+"&teacherid="+<?=$teacherid?>,
			type: 'POST',
			dataType: 'json',
			data: data,
		})
...
function search_course(){
	var data=$('#search').serialize();
	$.ajax({
		url: "<?=$zjjp?>teacher_knob/get_search_course?teacherid="+<?=$teacherid?>,
		type: 'POST',
		dataType: 'json',
		data: data,
	})
			...
			$('#tbody').empty();
				...
				$.each(r.data.c, function(i, v) {
					...
					if((i)%2==0){
						...
						str+='<label class="inline"><input '+checked+' onchange="setcourse('+v.id+')" value="'+v.id+'" type="checkbox" name="courseid"><span class="lbl">'+v.name+'</span></label></form></td>';
					}else{
						$.each(r.data.mc, function(ii, vv) {
							 if(vv.courseid==v.id){
							 	checked='checked="checked"';
						...
						str+='<input type="hidden" value="<?=$teacherid?>" name="teacherid" />';
						str+='<input id="setcourseid'+v.id+'" type="hidden" value="'+id2+'" name="id" />';
						str+='<label class="inline"><input '+checked+' onchange="setcourse('+v.id+')" value="'+v.id+'" type="checkbox" name="courseid"><span class="lbl">'+v.name+'</span></label></form></td></tr><tr>';
					}
				});
				$('#tbody').append(str);
				$('#next').attr({
...
function refresh(){
	window.location.href="http://www.sdyinc.com/master/teacherside/teacher_knob?teacherid=<?=$teacherid?>";
}
</script>