...
$form_action = $uri4 == 'edit_info' ? 'update_info' : 'insert_info';
	...
	<div class="col-xs-12">
		...
		<div>
				<h3 class="lighter block green"><?=!empty($info)?'编辑校区':'添加校区'?>
					<a href="/master/enrollment/buildingprice?buildingid=<?=$buildingid?>" title='返回上一级' class="pull-right ">
						<i class="ace-icon fa fa-reply light-green bigger-130"></i>
					</a>
					</h3>	
			<form class="form-horizontal" id="validation-form" method="post" action="/master/enrollment/buildingprice/insert_info">
				...
				<input type="hidden" name="roomid" value="<?=!empty($id)?$id:''?>">
	...
	$('#validation-form').validate({
					...
					submitHandler: function (form) {
						...
						.done(function(r) {
							if(r.state==1){
								pub_alert_success();
								window.location.href="<?=$zjjp?>buildingprice?buildingid=<?=$buildingid?>";
							}else{
								pub_alert_error();
							}
