										...
										<div class="widget-body">
											<div class="widget-main">
												<form class="form-inline" id="condition" method="post">
													...
													<input type="hidden" name="id" value="<?=!empty($acc_info->id)?$acc_info->id:''?>">
													<input type="hidden" name="userid" value="<?=!empty($acc_info->userid)?$acc_info->userid:''?>">
		...
		if(r.state==1){
			...
			$.each(r.data.room, function(k, v) {
				...
				if(v.is_reserve==0){
					...
					str+='<a class="btn btn-app btn-primary btn-sm" href="javascript:;" onclick="pub_alert_html(\'<?=$zjjp?>acc_in/adjust_acc_instudent?floor='+r.data.floor+'&bulidingid='+r.data.bulidingid+'&campusid='+r.data.campusid+'&userid='+r.data.userid+'&roomid='+v.id+'&id='+r.data.id+'&s=1\')"><i class="ace-icon glyphicon glyphicon-home"></i>'+v.name+'</a></div>';
