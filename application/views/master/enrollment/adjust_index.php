...
<ul class="breadcrumb">
	...
	<li>
		<a href="javascript:;">住宿预订</a>
	</li>
	<li class="active">住宿调剂</li>
</ul>
EOD;
?>		
	...
	<div class="col-xs-12">
		...
		<div>
					<h3 class="lighter block green"> &nbsp;
						<a href="/master/enrollment/acc_apply/index?&label_id=2" title='返回上一级' class="pull-right ">
										...
										<div class="widget-body">
											<div class="widget-main">
												<form class="form-inline" id="condition" method="post">
													...
													<input type="hidden" name="id" value="<?=$acc_info->id?>">
													<input type="hidden" name="userid" value="<?=$acc_info->userid?>">
		...
		if(r.state==1){
			...
			$.each(r.data.room, function(k, v) {
				...
				if(v.is_reserve==0){
					...
					str+='<a class="btn btn-app btn-primary btn-sm" href="javascript:;" onclick="pub_alert_html(\'<?=$zjjp?>acc_apply/adjust_allocation_room?floor='+r.data.floor+'&bulidingid='+r.data.bulidingid+'&campusid='+r.data.campusid+'&userid='+r.data.userid+'&roomid='+v.id+'&id='+r.data.id+'&s=1\')"><i class="ace-icon glyphicon glyphicon-home"></i>'+v.name+'</a></div>';
