...
function student_quick(){
	...
	var value=$('#value').val()
	$.ajax({
		url: '/master/charge/retreat_fee/get_student_quick?key='+key+'&value='+value,
		type: 'POST',
		dataType: 'json',
		data:{}
	})
	.done(function(r) {
		if(r.state==1){
			...
			$.each(r.data.stu, function(k, v) {
				...
				str+='<span style="float:right;"><a onclick="pub_alert_html(\'<?=$zjjp?>retreat_fee/retreat_fee_page?userid='+v.id+'&s=1\')" href="javascript:;" class="btn btn-xs"><i class="ace-icon fa fa-bolt bigger-110"></i>退费</a></span></td></tr>';
