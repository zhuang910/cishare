						...
						<form  id="outline" method="post">
							<div class="form-group">
								<div class="col-xs-12 col-sm-12">
										<div class="wysiwyg-editor" id="editor1"><?=!empty($t_c_info)&&!empty($t_c_info['outline']) ? $t_c_info['outline'] : ''?></div>
										 <input type="hidden" name="id" id="id" value="<?=!empty($t_c_info)&&!empty($t_c_info['id']) ? $t_c_info['id'] : ''?>">
								</div>
							</div>
...
function tijiao_outline(){
	...
	$.ajax({
		url: "/master/teacher/teacher_course/edit_outline",
		type: 'POST',
		dataType: 'json',
		data: data+'&outline='+html
	})
	.done(function(r) {
		if(r.state==1){
			window.location.href="<?=$zjjp?>teacher_course?teacherid=<?=$t_c_info['teacherid']?>";
