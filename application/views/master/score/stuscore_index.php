							...
							<div id="tables" class="widget-box transparent collapsed">
								<div class="widget-body">
									...
									<form id='scorearr'>
										...
										<a id='tijiao' onclick="score_submit()" class="btn btn-info" type="button" >
...
function sure(){
	...
	.done(function(r) {
		if(r.state==1){
            ...
            if(r.data.info!=''){
                var str='';
                $.each(r.data.info, function(kk, vv) {
                     str+='<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong>提示!</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                        str+='<span>代课老师：</span> <span id="one">'+vv.name+'</span <span id="two">'+vv.phone+'</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>邮箱：</span><span id="three">'+vv.email+'</span></div>';
                });
                $('#tea_info').append(str);
            }else{
                var str='<div class="alert alert-warning"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong>提示!</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                str+='<span>该课程没有老师带</span></div>';
                $('#tea_info').append(str);
            }
			...
			$.each(r.data.stu, function(k, v) {
				...
				strr='<tr><td width="200">'+v.studentid+'</td><td width="200">'+v.name+'</td><td>'+v.enname+'</td><td><input name="'+v.id+'[]" type="text" data-score="true"></td><td><input name="'+v.id+'[]" type="text" data-score="true"></td><td></td></tr>';
				$.each(r.data.scoreinfo, function(kk, vv) {
					 if(v.id==vv.studentid&&vv.courseid==cid&&vv.term==term&&vv.squadid==sid&&vv.scoretype==type){
					 	strr='<tr><td width="200">'+v.studentid+'</td><td width="200">'+v.name+'</td><td>'+v.enname+'</td><td>'+vv.score+'</td><td>'+vv.show_score+'</td><td><a class="red" onclick="del('+vv.id+')" href="javascript:;"><i class="ace-icon fa fa-trash-o bigger-130"></i></a></td></tr>';
					 }
				});
				str+=strr;
			}); 
	...
	if(num==1){
		...
		.done(function(r) {
			if(r.state==1){
				...
				pub_alert_success();
				$('#tijiao').html('<i class="ace-icon fa fa-check bigger-110"></i>提  交');
