...
<div class="width1024">
  <div class="wap_box p30">
    <h1 class="clearfix"><span class="fl"><?=lang('my_add_activity')?></span><em class="title_btn fr"><a href="/<?=$puri?>/society/activity/add"><?=lang('add_activity')?></a></em></h1>
    <div class="tab pt30">
	 <?php //$this->load->view('society/activity_nav.php');?>
      ...
      <div class="Switch_bg">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
		  ...
		  ?>
		   <tr>
            <td align="left"><a href="/<?=$puri?>/society/activity/detail?id=<?=$v['id']?>"><?php if($puri == 'cn'){echo $v['ctitle'];}else{echo $v['etitle'];}?></a><br />
            ...
            <td align="left">
				<a href="/<?=$puri?>/society/activity/detail?id=<?=$v['id']?>"><?=lang('detail')?></a> / 
				...
				?>
					...
					<a href="/<?=$puri?>/society/activity/add_c?id=<?=$v['id']?>"><?=lang('activity_add_content')?></a> /
				<?php }else if($v['state'] == 1){?>
				  <a href="/<?=$puri?>/society/activity/ckuser?id=<?=$v['id']?>"><?=lang('activity_ck_user')?>(<?=!empty($count) && !empty($count[$v['id']])?$count[$v['id']]:0?>)</a>
				<?php }else{?>
				--
				<?php }?>
			</td>
		...
		var d = dialog({
				...
				ok: function () {
					...
					setTimeout(function () {
					 ...
					 $.post('/<?=$puri?>/society/activity/js_activity',{id:id},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
		...
		var d = dialog({
				...
				ok: function () {
					...
					setTimeout(function () {
					 ...
					 $.post('/<?=$puri?>/society/activity/del_activity',{id:id},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
...
<script type="text/javascript">
	...
	function more(page){
		...
		if(page <= pagecount){
			$.ajax({
				type:'GET',
				url:'/<?=$puri?>/society/activity/get_activity_launch?page='+page,
				success:function(r){
					if(r.state == 1){
						$('#loading').show();
