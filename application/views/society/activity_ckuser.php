...
<div class="width1024">
  <div class="wap_box p30">
    <h1 class="clearfix"><span class="fl"><?=lang('my_add_activity')?></span><em class="title_btn fr"><a href="/<?=$puri?>/society/activity/launch">
      <?=lang('activity_return')?>
      </a></em><em class="title_btn fr"><a href="/<?=$puri?>/society/activity/add"><?=lang('add_activity')?></a></em></h1>
    <div class="tab pt30">
		...
		if(activityid != '' && userid != ''){
			var d = dialog({
				...
				ok: function () {
					...
					var score = $('#score').val();
					$.post('/<?=$puri?>/society/activity/activity_isapply_score',{userid:userid,activityid:activityid,score:score},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
		...
		if(activityid != '' && userid != ''){
			var d = dialog({
				...
				ok: function () {
					...
					$.post('/<?=$puri?>/society/activity/activity_isapply_check',{userid:userid,activityid:activityid,state:state,auditopinion:auditopinion},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
		...
		if(activityid != '' && userid != ''){
			var d = dialog({
				...
				ok: function () {
					...
					$.post('/<?=$puri?>/society/activity/activity_join_set',{userid:userid,activityid:activityid,isjoin:isjoin},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
		...
		var d = dialog({
				...
				ok: function () {
					...
					setTimeout(function () {
					 ...
					 $.post('/<?=$puri?>/society/activity/activity_cancel_join',{id:id},function(msg){
						  if(msg.state == 1){
							var d = dialog({
								content: ''+msg.info+''
							});
							...
							setTimeout(function () {
								d.close().remove();
								window.location.href='/<?=$puri?>/society/activity/index';
							}, 5000);
...
<script type="text/javascript">
	...
	function more(page){
		...
		if(page <= pagecount){
			$.ajax({
				type:'GET',
				url:'/<?=$puri?>/society/activity/get_activity_index?page='+page,
				success:function(r){
					if(r.state == 1){
						$('#loading').show();
