...
<script>
        ...
        /*前一步*/
        function Previous(){
          window.location.href='/<?=$puri?>/scholarshipgrf/fillingoutforms/apply?applyid='+applyid;
        ...
        function Next(){
          //验证表单
           var  ajaxCallUrl= '/<?=$puri?>/scholarshipgrf/apply/validatorsaveMateerial?applyid='+applyid;
        $.ajax({
               cache: true,
               type: "GET",
               url:ajaxCallUrl,
               ...
               success: function(r) {
                  if(r.state == 1){
                    window.location.href='/<?=$puri?>/scholarshipgrf/apply/submit_application?applyid='+applyid;
                  }else{
						 var d = dialog({
							content: ''+r.info+''
						});
         ...
         //  window.location.href='/apply/apply/make_paymeznt?applyid='+applyid;
                ...
                function getPr(w){
                  if(is_upload === true){
                    ...
                    setTimeout(function(){
                      $.ajax({
                        type:'get',
                        url:'/<?=$puri?>/scholarshipgrf/bigupload/getprogress',
          ...
          function filehander(divthis,at_id,count){
                 var url = '/<?=$puri?>/scholarshipgrf/apply/upload_files';
            ...
            $('#fileupload_'+at_id).fileupload({
                 ...
                 // gethtml+="</div></div></div>";
                  ...
                  win.close();
                  var up_mongodb_url="/<?=$puri?>/scholarshipgrf/apply/save_upload_atta";
				   ...
				   new_upload_img.on('mouseover mouseout',function(event) {
					  ...
					  if (event.type == 'mouseover') {
					   ...
					   $(".delete-action a").on('click',function(){
							  ...
							  var delurl="/<?=$puri?>/scholarshipgrf/apply/delFiles";
							  var d = dialog({
										title: '<?=lang('welcome')?>',
