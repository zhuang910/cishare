    ...
    <div class="tab pt30">
      ...
      <div class="Switch_bg">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
		  ...
		  ?>
		   <tr>
            <td align="left"><a href="/<?=$puri?>/scholarshipgrf/index/scholarship_detail?id=<?=$val['id']?>"><?php if($puri == 'en'){?><?=!empty($val['entitle'])?$val['entitle']:''?><?php }else{?><?=!empty($val['title'])?$val['title']:''?><?php }?></a></td>
            <td align="left"><a href="/<?=$puri?>/scholarshipgrf/apply?id=<?=cucas_base64_encode($val['id'])?>&applyid=<?=$applyid?>"><?=lang('apply_1')?></a><br /></td>
