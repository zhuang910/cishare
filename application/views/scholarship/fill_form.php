...
<div class="width_925 clearfix applyonline-main">
	...
	<div class="applyonline-2-main">
		...
		<form method="post" id="myform" action="/<?=$puri?>/scholarshipgrf/fillingoutforms/save/<?=$cid?>" onsubmit="return scroll_to_error();">
			<?=$html_form?>
...
function do_save(){
	...
	$.ajax({
		url:'/<?=$puri?>/scholarshipgrf/fillingoutforms/save/<?=cucas_base64_encode($apply_info['id'])?>',
		type:'post',
		data:data,
		dataType:'json',
		beforeSend:function(){
...
$(function(){
	$("#myform").ajaxForm({
		url:'/<?=$puri?>/scholarshipgrf/fillingoutforms/save/<?=cucas_base64_encode($apply_info['id'])?>/ischeck',
		dataType:'json',
		success:function(r){
			if(r.state == 1){
				var d = dialog({
